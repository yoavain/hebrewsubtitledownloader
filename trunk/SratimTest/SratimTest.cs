...
namespace SratimTest
{
  ...
  public class SratimTest
  {
    ...
    {
      const string strRegex = @"<div style=\""[^>]*?>\s*?<a href=\"".*view.php\?id=(?<movie_id>\d+)&amp;q=(?<movie_query>[^\""]*)\""\s*?title=\""(?<movie_hebrew>[^(]*)\((?<movie_year>[^\\)]*)\)\""\>";
      const RegexOptions myRegexOptions = RegexOptions.None;
      var myRegex = new Regex(strRegex, myRegexOptions);
      const string strTargetString = @"<div style=""vertical-align:top;padding-left:14px;padding-bottom:12px;""><a href=""view.php?id=11813&amp;q=batman+begins"" title=""באטמן מתחיל (2005)"">";
